...
/**
 * Which HeyGen object this avatar is — NOT which engine renders it.
 * Both render on v1/v2 (`/v2/video/generate`).
 */
export type AvatarEngine = "talking_photo" | "photo_avatar_group";

/**
 * The HeyGen v3 engine the operator CHOSE for this avatar. Avatar V exists only on
 * v3 (`POST /v3/videos`), so this doubles as "render this one via v3".
 *
 * This is INTENT, not capability — a deliberate distinction:
 *   • intent     (ours)    — stable, so we persist it.
 *   • capability (HeyGen's) — `supported_api_engines`, derived from server state we
 *     cannot see or predict, and demonstrably mutable. NEVER persisted; always read
 *     live from the current API response, which is the only authority.
 *
 * Storing capability would let a months-old cached "eligible" 400 on every beat,
 * mid-run, after the voiceover is already paid for.
 */
...
export interface Avatar {
  ...
  /**
   * "1" when this row only REFERENCES an avatar the operator created on HeyGen
   * (import-by-id). We never created that asset, so deleting this row must not
   * delete it from their HeyGen account. NULL = created by us (ours to clean up).
   */
  imported: string | null;
  /**
   * "avatar_v" when the operator chose HeyGen's Avatar V engine (rendered via v3).
   * NULL = rendered via v2, where `use_avatar_iv` picks Avatar IV vs Legacy.
   * Intent, never a cached capability — see ApiEngine.
   */
...
export interface CreateAvatarInput {
  ...
  /** true = we only reference an operator-owned HeyGen avatar; never delete it remotely. */
  imported?: boolean;
...
// isRenderableAvatar() lived here: a Stage-1 guard that made an Avatar V avatar
// unselectable for runs while only the v2 renderer existed. Stage 2 renders Avatar V on
// v3, so every avatar is renderable and the guard is gone. What replaced it is the live
// eligibility check in /api/studio + resumeStudioPipeline — a stored api_engine says
// what was CHOSEN, never that HeyGen still grants it.

/**
 * May we delete this avatar's asset from the operator's HeyGen account?
 *
 * Only for one we CREATED there (`imported` unset) — an imported row merely
 * references an avatar the operator made themselves, and removing it from our
 * library must never destroy their asset. Beyond provenance: we only ever create
 * a deletable slot for the talking_photo engine, and only once it has a handle.
 */
export function ownsHeygenAsset(
  avatar: Pick<Avatar, "imported" | "engine" | "heygen_id">
): boolean {
  return avatar.imported !== "1" && avatar.engine === "talking_photo" && !!avatar.heygen_id;
}
