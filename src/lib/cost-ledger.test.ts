...
describe("cost ledger integration", () => {
  it("records every provider and aggregates per-run buckets + total", () => {
    ...
    ledger.recordGemini(RUN, "geminiText", 1_000_000, 0, "gemini-2.5-flash"); // 1M in × 0.30 = 0.30
    ledger.recordGemini(RUN, "geminiVision", 0, 1_000_000, "gemini-2.5-flash"); // 1M out × 2.50 = 2.50
  ...
  it("overview total = sum of every run's total (the /api/costs computation)", () => {
    ...
    expect(agg.get(RUN)!.total).toBeCloseTo(7.98, 6); // prior run unaffected

    const overviewTotal = [...agg.values()].reduce((s, x) => s + x.total, 0);
    expect(overviewTotal).toBeCloseTo(7.98 + 0.95, 6); // 8.93
  });
