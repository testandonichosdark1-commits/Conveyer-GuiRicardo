...
export async function assembleVideo(
  ...
  // Final mastering: normalize delivered audio to the loudness target (shared with the
  // studio pipeline so every render path lands at the same level). Safe no-op if off.
...
/** Points fluent-ffmpeg at the resolved ffmpeg/ffprobe (FFMPEG_PATH -> bundled ./bin -> system PATH). */
function ensureFfmpegPaths(): void {
  const ffmpegBin = resolveFfmpeg();
  // Bare "ffmpeg"/"ffprobe" are fluent-ffmpeg's own defaults — only override when
  // we resolved a real path (FFMPEG_PATH or a bundled ./bin binary).
  if (ffmpegBin !== "ffmpeg") ffmpeg.setFfmpegPath(ffmpegBin);
  const ffprobeBin = resolveFfprobe();
  if (ffprobeBin !== "ffprobe" && fs.existsSync(ffprobeBin)) ffmpeg.setFfprobePath(ffprobeBin);
}
...
/** Why a strict audio probe rejected a file — lets a caller map to an exact user message. */
export type AudioProbeFailure = "unreadable" | "no_audio_stream" | "no_duration";

/** Thrown by probeAudioStrict(). `reason` is the machine-readable discriminator. */
export class AudioProbeError extends Error {
  readonly reason: AudioProbeFailure;
  constructor(reason: AudioProbeFailure, message: string) {
    super(message);
...
/** What a strict probe learned about a media file's audio. */
export interface AudioProbe {
  /** Seconds, always finite and > 0 — never a file-size guess. */
  durationSec: number;
  codec: string | null;
...
/**
 * STRICT audio probe — the validating counterpart to probeDurationSafe().
 *
 * probeDurationSafe() is deliberately unfailable: on any error it returns
 * `size / 16000` and, failing that, 1. That is right for the pipeline (a bad probe
 * must never kill a render mid-run) but makes it useless for VALIDATION — a corrupt
 * upload, a PDF renamed to .mp3, or a silent video would all come back with a
 * plausible-looking duration and be accepted.
 *
 * So this one fails loudly and never estimates:
 *   - ffprobe cannot read the file            → "unreadable"
 *   - the file has no audio stream at all     → "no_audio_stream"
 *   - no finite, positive container duration  → "no_duration"
 *
 * Accepting any ffprobe-decodable container (mp3/wav/m4a/flac/ogg/… and the audio
 * track of a video) is intentional: the file extension is not evidence, the probe is.
 *
 * KNOWN LIMIT (measured, not theoretical): a TRUNCATED mp3 still passes, reporting the
 * duration declared in its header rather than the bytes actually present. ffprobe trusts
 * container metadata, so no probe can catch that cheaply. It is benign here — the file is
 * genuinely decodable, and the duration that ends up driving timings is re-probed from the
 * transcoded audio later, not taken from this number.
 *
 * probeDurationSafe() is untouched — both exist, for opposite jobs.
 */
export async function probeAudioStrict(filePath: string): Promise<AudioProbe> {
  ensureFfmpegPaths();
  const data = await new Promise<ffmpeg.FfprobeData>((resolve, reject) => {
    try {
      ffmpeg.ffprobe(filePath, (err, d) => {
        if (err) {
          reject(new AudioProbeError("unreadable", `ffprobe could not read this file: ${String(err).slice(0, 200)}`));
          return;
        }
        resolve(d);
      });
    } catch (e) {
      // ffprobe binary missing / spawn failure throws synchronously rather than calling back.
      reject(new AudioProbeError("unreadable", `ffprobe could not be run: ${(e as Error).message.slice(0, 200)}`));
    }
  });

  const audio = (data.streams ?? []).find((s) => s.codec_type === "audio");
  if (!audio) {
    throw new AudioProbeError("no_audio_stream", "This file has no audio track.");
  }

  const durationSec = Number(data.format?.duration);
  if (!Number.isFinite(durationSec) || durationSec <= 0) {
    throw new AudioProbeError("no_duration", "This file's duration could not be determined — it may be corrupt.");
  }

  // ffprobe reports these as numbers or numeric strings depending on build; normalize,
  // and report null rather than a wrong number when a field is absent.
  const sampleRateHz = Number(audio.sample_rate);
  const channels = Number(audio.channels);
  return {
    durationSec,
    codec: audio.codec_name ?? null,
    sampleRateHz: Number.isFinite(sampleRateHz) && sampleRateHz > 0 ? sampleRateHz : null,
...
function concatWithCrossfade(
  ...
  // xfade offset = current output length − fade. Clamp the fade per-boundary to
  // min(fadeDur, running output length, next clip length): a clip SHORTER than
  // fadeDur used to drive `offset` negative (overlap starting before the source
  // exists → the decoder shows black frames at the join). Clamping keeps every
  // offset ≥ 0 and every fade within both clips, so a short clip just hard-cuts
  // (tiny fade) instead of tearing to black.
  let runningDur = clips[0].durationSec;
  for (let i = 1; i < clips.length; i++) {
    const eff = Math.max(0, Math.min(fadeDur, runningDur, clips[i].durationSec));
    const offset = Math.max(0, runningDur - eff);
    const vOut = `v${i}`;
    const aOut = `a${i}`;
    videoChain += `[${lastV}][${i}:v]xfade=transition=fade:duration=${eff.toFixed(3)}:offset=${offset.toFixed(3)}[${vOut}];`;
    audioChain += `[${lastA}][${i}:a]acrossfade=d=${eff.toFixed(3)}[${aOut}];`;
    runningDur = runningDur + clips[i].durationSec - eff;
    lastV = vOut;
    lastA = aOut;
  }
  // Strip trailing ;
...
export async function assembleSingleShot(
  ...
  // Final mastering: normalize delivered audio to the loudness target (shared helper).
  masterLoudness(runId, finalPath);
  log(runId, "success", `Final video: ${finalPath}`, { stage: "assemble" });
