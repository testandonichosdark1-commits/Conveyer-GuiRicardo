 ...
 */
async function transcribeWithGroqWhisper(runId: string, audioPath: string): Promise<TranscriptWord[]> {
  ...
  // Cost Monitoring — billed per hour of audio, and recorded BEFORE the empty-words
  // check: Groq charges for the call whether or not it returned usable timestamps.
  // This call site was unmetered, so single-shot TTS runs under-reported Groq spend.
  const billedSec =
    typeof json.duration === "number" && json.duration > 0
      ? json.duration
      : json.words?.length
        ? (json.words[json.words.length - 1]?.end ?? 0)
        : 0;
  if (billedSec > 0) recordGroqTranscription(runId, billedSec);
