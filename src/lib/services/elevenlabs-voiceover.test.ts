...
describe("isTransientElevenLabsError", () => {
  ...
  it("does NOT retry our own abort-timeout — re-sending would re-bill an already-generated chunk", () => {
    expect(isTransientElevenLabsError("ElevenLabs timeout after 600s")).toBe(false);
  });
  it("treats genuine network drops as transient", () => {
    expect(isTransientElevenLabsError("fetch failed")).toBe(true);
    expect(isTransientElevenLabsError("ECONNRESET")).toBe(true);
    expect(isTransientElevenLabsError("ETIMEDOUT")).toBe(true);
  });
