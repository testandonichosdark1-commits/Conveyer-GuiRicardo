 ...
 *
 * Version-agnostic on purpose: these helpers take a caller-supplied path against
 * api.heygen.com, and v1, v2 and v3 all share the same host and `X-Api-Key` auth. So
 * Avatar IV/Legacy (v2 `/v2/video/generate`) and Avatar V (v3 `/v3/videos`) both go
 * through heygenPost/heygenGet — nothing here is per-version.
 *
 * uploadAsset() posts a RAW body to upload.heygen.com/v1/asset. Its `data.id` works as
 * an `audio_asset_id` on BOTH v2 and v3 (verified: the asset resolves via
 * GET /v3/assets/{id} and rendered end-to-end), which is why there is no v3 multipart
 * upload here. /v3/assets exists and returns `data.asset_id` — a different shape — but
 * we have no need of it.
 *
 * See docs/DESIGN.md for the v2 surface; CLAUDE.md for the probed v3 contract.
 *
...
/**
 * Authenticated DELETE. Used to free HeyGen resources (e.g. talking photos —
 * plans cap them at a few slots, so deleting an avatar here must release its
 * slot there). Throws on HTTP error; callers treat cleanup as best-effort.
 */
export async function heygenDelete(pathWithQuery: string): Promise<void> {
  const resp = await fetchTimeout(`${API_BASE}${pathWithQuery}`, {
    method: "DELETE",
    headers: { "X-Api-Key": heygenKey(), Accept: "application/json" },
  }, JSON_TIMEOUT_MS);
  if (!resp.ok) {
    throw new Error(`HeyGen DELETE ${pathWithQuery} ${resp.status}: ${(await resp.text()).slice(0, 200)}`);
  }
}
