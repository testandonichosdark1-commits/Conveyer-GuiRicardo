...
/**
 * GDRIVE_SYNC_ENABLED is a TRI-state, not a boolean, and the third state is the
 * whole point:
 *
 *   "1"  → auto-upload on
 *   "0"  → the user deliberately turned it OFF
 *   ""   → never configured (the shipping default)
 *
 * Only `""` may be auto-enabled on a first connection. Before this distinction
 * existed the checkbox wrote `""` when unticked, so "deliberately off" and "never
 * touched" were byte-identical and there was no safe way to enable anything
 * automatically without overriding someone's explicit choice.
 *
 * Reading it through getSetting() (not the raw row) is deliberate: getSetting falls
 * back to process.env for an empty value, so an operator who sets
 * GDRIVE_SYNC_ENABLED in the environment has also expressed a choice, and we leave
 * it alone.
 */
function syncNeverConfigured(): boolean {
  return getSetting("GDRIVE_SYNC_ENABLED") === "";
}

/**
 * Second leg of OAuth: trade code for tokens, store refresh_token + email.
 *
 * On a user's FIRST connection this also switches auto-upload on. Connecting Drive
 * and enabling uploads used to be two unrelated actions on two different pages, and
 * nothing linked them — so the common outcome was a correctly connected account that
 * silently never uploaded anything, with no error to explain it.
 *
 * It is enabled only when BOTH hold:
 *   - no refresh token was on file (this is a first-ever connection, not a reconnect
 *     or an account switch), and
 *   - the toggle has never been configured (see syncNeverConfigured).
 *
 * The first condition is what protects EXISTING installs: anyone already connected
 * has a token, so their settings are never touched no matter what they hold. The
 * second is what protects a deliberate opt-out from being undone by a reconnect.
 *
 * Returns whether it enabled the toggle, so the UI can say so honestly rather than
 * claiming uploads are on when they are not.
 */
export async function exchangeCodeForTokens(code: string): Promise<{ email: string; autoEnabledSync: boolean }> {
  const oauth = getOAuth2Client();
  if (!oauth) throw new Error("OAuth client not configured");

  // Read BEFORE the write below overwrites it.
  const hadPriorConnection = !!getSetting("GDRIVE_REFRESH_TOKEN");
  ...
  const autoEnabledSync = !hadPriorConnection && syncNeverConfigured();
  if (autoEnabledSync) setSetting("GDRIVE_SYNC_ENABLED", "1");

  return { email, autoEnabledSync };
}
 ...
 * settings, they're returned as-is. Otherwise creates `Conveyer Grok/Final Videos`
 * and `Conveyer Grok/Clips Library` in the user's Drive root and persists the IDs.
 * Separate root folder from Hum Conveyer so the two projects stay isolated in Drive.
 */
export async function ensureTopLevelFolders(): Promise<{
  ...
  if (!finalId || !clipsId) {
    const rootFolder = await findOrCreateFolder("Conveyer Grok");
    if (!finalId) {
