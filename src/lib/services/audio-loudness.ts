...
import { resolveFfmpeg, STRIP_TOOLCHAIN_TAGS } from "../ffmpeg-bin";
...
export function masterLoudness(runId: string, finalPath: string): void {
  ...
  const r = spawnSync(
    ...
    [
      ...
      // This pass rewrites final.mp4 AFTER assembly, so it is the last writer whenever
      // loudness mastering is on (the default) — it has to re-apply the tag strip or it
      // would stamp ffmpeg's encoder tags straight back onto the delivered file.
