 ...
 */
import { AI84_MODELS } from "@/lib/providers";
...
export interface Field {
  ...
  multiline?: boolean;
  /**
   * If set, renders a <select> dropdown instead of a text input. A plain string is both
   * the stored value and the label; `{value,label}` separates them, so a friendly label
   * ("Images only — 💵 Cheapest") can map to a canonical stored value ("image").
   */
...
export const ALL_GROUPS: Group[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "FALLBACK_AI_MEDIA",
        label: "AI Fallback Media",
        desc: "When a scene has no findable real footage and AI fallback kicks in, this is what it may generate. AI VIDEO is far pricier than AI images, so the default is images only. This affects ONLY the real-footage fallback — normal AI generation, avatars and thumbnails are untouched.",
        examples: "Images only = cheapest & safest (default)  ·  Videos only = priciest  ·  Images + Videos = the old behaviour",
        options: [
          { value: "image", label: "Images only — 💵 Cheapest" },
          { value: "video", label: "Videos only — 🎥 Highest quality" },
          { value: "both", label: "Images + Videos — ⭐ Best quality" },
        ],
      },
      {
        key: "PEXELS_API_KEY",
      ...
      {
        key: "WIGOLO_URL",
        desc: "Address of the local wigolo daemon — powers the 'Wigolo (web photos)' footage source (free web-wide PHOTO search; wigolo has no video at all). The source must also be ticked under Real footage sources. Empty = inert.",
        examples: "http://127.0.0.1:3477",
      },
      {
        key: "WIGOLO_API_TOKEN",
        desc: "Bearer token for the wigolo daemon. Only needed when it listens on something other than loopback — on loopback with no token configured the API is open.",
        examples: "Leave empty for a local daemon",
      },
      {
        key: "WIGOLO_EXCLUDE_DOMAINS",
        desc: "Domains dropped from wigolo results. The match is on the PAGE domain (subdomains included), never the image CDN — 'alamy.com' works, 'c8.alamy.com' does nothing. Stock agencies serve watermarked previews, so this list is what keeps watermarks out of the finished video.",
        examples: "alamy.com,gettyimages.com,shutterstock.com",
      },
      {
        key: "WIGOLO_MIN_PX",
        desc: "Minimum width AND height for a wigolo photo. Stock APIs guarantee a usable size; open-web search does not, so this is the size floor.",
        examples: "700 (default)",
      },
      {
        key: "WIGOLO_BIN",
        desc: "Path to the wigolo executable, used only to auto-start the daemon. Empty = look for 'wigolo' on PATH, then node_modules/.bin. Nothing is ever installed automatically — if it isn't found, the app starts normally and the source simply stays unavailable.",
        examples: "/usr/local/bin/wigolo  ·  leave empty on a normal install",
      },
      {
  ...
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        desc: "Specific model id. For Google, the `-latest` alias auto-tracks the current stable Flash. For Anthropic use the full model id. Retired Gemini 2.0/2.5 ids are no longer accepted — if saved they're auto-upgraded to the matching Gemini 3 model.",
        examples: "gemini-3.5-flash, gemini-3.1-flash-lite, gemini-3.1-pro-preview",
      },
    ],
  },
  {
    ...
    fields: [
      {
        ...
        examples: "heygen  /  69labs  /  elevenlabs  /  genaipro  /  openai  /  minimax",
        options: ["heygen", "69labs", "elevenlabs", "genaipro", "openai", "minimax"],
      },
      {
        key: "TTS_MODE",
      ...
      },
      {
        ...
        desc: "Which engine 69labs uses behind its gateway — only read when TTS_PROVIDER=69labs. `edgetts` (default) expects an Edge voice NAME (en-US-GuyNeural). `elevenlabs` expects an ElevenLabs voice ID. `voice-clone` expects a 69labs voice-clone ID and posts to /voice-clones/generate instead. Sent as `voiceProvider` on /tts/generate — a mismatch between this and TTS_VOICE_ID is what produces \"This Edge TTS voice ID was not found.\" Same setting as the 69labs voice-engine picker on the main Settings page.",
        examples: "edgetts  /  elevenlabs  /  voice-clone",
        options: [
      ...
      {
        key: "TTS_VOICE_ID",
        desc: "Voice id for the NON-HeyGen providers (69labs / ElevenLabs / OpenAI). HeyGen ignores this — it uses HEYGEN_VOICE_ID. For ElevenLabs: a voice ID from their library. For OpenAI: a voice name like `alloy`. For 69labs the expected FORM depends on TTS_VOICE_PROVIDER above: an Edge voice name, an ElevenLabs voice ID, or a 69labs voice-clone ID.",
        examples: "Edge: en-US-GuyNeural  ·  ElevenLabs: 21m00Tcm4TlvDq8ikWAM  ·  OpenAI: alloy, onyx, nova",
      },
      {
        key: "TTS_MODEL",
      ...
      {
        key: "AI84_API_KEY",
        desc: "AI84 (api.ai84.pro) API key. Required when VOICEOVER_PROVIDER = ai84 (avatar documentary mode). AI84 is an ElevenLabs/MiniMax reseller with an async TTS task API; it has NO native word timestamps, so word timing is recovered via Groq Whisper (set GROQ_API_KEY) or a proportional fallback — same as every non-ElevenLabs provider.",
        examples: "sk-user-xxxxxxxx (from ai84.pro → API key)",
      },
      {
        key: "AI84_VOICE_ID",
        desc: "AI84 narration voice_id — an ElevenLabs shared-voice id. A channel profile's voice_id overrides this per channel.",
        examples: "JBFqnCBsd6RMkjVDRZzb",
      },
      {
        key: "AI84_MODEL",
        desc:
          "AI84 model_id — the DEFAULT model, and the tier you're billed at. " +
          "AI84 fronts two engines: `eleven_*` models run on ElevenLabs, `speech-*` models run on " +
          "MiniMax, and voices are NOT shared between them (voices you cloned yourself, ids like " +
          "`user_1234_voice_1786…`, exist ONLY on MiniMax). " +
          "**This setting no longer decides which engine a video uses — the voice does.** Each " +
          "video and each channel can have its own voice, and the engine follows it, so channels " +
          "on both engines work at the same time. What this picks is the TIER within an engine " +
          "(2.8 HD vs 2.6 turbo, multilingual vs flash), and it stays in charge whenever a voice " +
          "can't be found in either library. MiniMax bills slightly more (5 credits per 4 " +
          "characters vs 4).",
        examples: "eleven_multilingual_v2  (ElevenLabs)  /  speech-2.8-hd  (MiniMax, for cloned voices)",
        // Generated from AI84_MODELS so this list cannot drift from the one the runtime
        // routes on. The label says the engine out loud — the option text is the only
        // place an operator learns the rule.
        options: AI84_MODELS.map((m) => ({
          value: m.id,
          label: `${m.backend === "minimax" ? "MiniMax" : "ElevenLabs"} · ${m.label}`,
        })),
      },
      {
        key: "COST_AI84_USD_PER_CREDIT",
        desc: "USD price of ONE AI84 credit (for the Costs page). AI84 bills in credits and doesn't publish a $/credit rate, so set it from your plan. Blank/0 → cost rows still record the credit count but show €0.00.",
        examples: "e.g. 0.00002",
      },
      {
        key: "AI33_API_KEY",
        desc:
          "ai33.pro (product name: OpenSpeaker) API key. Required when VOICEOVER_PROVIDER = ai33. " +
          "ai33 is a reseller fronting SIX engines behind one key and one credit balance — clone, " +
          "elevenlabs, minimax, fishaudio, edge and vbee — of which Edge and Vbee are voices this app " +
          "has no other way to reach. It returns audio only, so word timing is recovered via Groq " +
          "Whisper (set GROQ_API_KEY) or a proportional fallback, same as every non-ElevenLabs provider.",
        examples: "From ai33.pro → API (keys are issued to supporters)",
      },
      {
        key: "AI33_VOICE_ID",
        desc:
          "ai33 narration voice_id, written **\"<engine>:<id>\"** — the engine is part of the id, which " +
          "is why ai33 needs no engine or model setting at all. Use “Load voices” on the main Settings " +
          "page: it lists all six engines together and writes the full id for you. A channel profile's " +
          "voice_id overrides this per channel.",
        examples: "edge:en-US-GuyNeural  /  elevenlabs:JBFqnCBsd6RMkjVDRZzb  /  clone:<your voice>",
      },
      {
        key: "AI33_BASE_URL",
        desc:
          "ai33 API host. LEAVE THIS BLANK unless ai33 tells you otherwise — blank means " +
          "https://api.openspeaker.ai, which is the documented host. It exists because ai33.pro and " +
          "openspeaker.ai are the same product served under two names, and which one a given account " +
          "is issued against could not be verified here.",
        examples: "(blank)  /  https://api.openspeaker.ai",
      },
      {
        key: "COST_AI33_USD_PER_CREDIT",
        desc:
          "USD price of ONE ai33 credit (for the Costs page). ai33 sells credit packs (about $5 per " +
          "1,000,000 premium credits) but publishes no per-engine rate, and its engines are not priced " +
          "alike — so set this from your own pack. Blank/0 → cost rows still record the credit count " +
          "but show €0.00.",
        examples: "e.g. 0.000005",
      },
      {
        key: "FISHAUDIO_API_KEY",
        desc: "Fish Audio (fish.audio) API key. Required when VOICEOVER_PROVIDER = fishaudio. Fish returns audio only, so word timing is recovered via Groq Whisper (set GROQ_API_KEY) or a proportional fallback — same as every non-ElevenLabs provider.",
        examples: "Create one at fish.audio → API Keys",
      },
      {
        key: "FISHAUDIO_VOICE_ID",
        desc: "Fish Audio voice — the MODEL id sent as `reference_id`. Use “Load voices” on the Settings page to pick one (it lists your own voices first, then the public library); the button writes this id for you. A channel profile's voice_id overrides it per channel.",
        examples: "802e3bc2b27e49c2995d23ef70e6ac89",
      },
      {
        key: "FISHAUDIO_MODEL",
        desc: "Fish Audio backend model, sent as the `model` HTTP header. `s2.1-pro` (default) / `s2-pro` / `s1` all cost $15 per million UTF-8 bytes; `s2.1-pro-free` is the free developer tier (set COST_FISHAUDIO_USD_PER_1M_BYTES to 0 if you use it).",
        examples: "s2.1-pro  /  s2-pro  /  s1  /  s2.1-pro-free",
        options: ["s2.1-pro", "s2-pro", "s1", "s2.1-pro-free"],
      },
      {
        key: "COST_FISHAUDIO_USD_PER_1M_BYTES",
        desc: "USD per MILLION UTF-8 BYTES of narration text (Fish Audio's billing unit — bytes, not characters). List price is $15.00. Note that non-Latin scripts use 2–3 bytes per character, so a Russian or Japanese script costs proportionally more than its character count suggests.",
        examples: "15.00  (0 for the s2.1-pro-free tier)",
      },
      {
        key: "HUME_API_KEY",
        desc: "Hume AI (hume.ai) API key for Octave TTS, sent as the X-Hume-Api-Key header. Required when VOICEOVER_PROVIDER = hume. Hume returns audio only, so word timing is recovered via Groq Whisper (set GROQ_API_KEY) or a proportional fallback.",
        examples: "From app.hume.ai → API keys",
      },
      {
        key: "HUME_VOICE_ID",
        desc: "Hume voice UUID. Use “Load voices” on the Settings page — it merges Hume's shared Voice Library with your own custom voices into one list, because a voice referenced by id resolves against both and needs no provider stored alongside it. A channel profile's voice_id overrides it per channel.",
        examples: "9e068547-5ba4-4c8e-8e03-69282a008f04",
      },
      {
        key: "HUME_VERSION",
        desc: "Octave model generation. Leave EMPTY to use Hume's own default (recommended). Octave 1 = English/Spanish; Octave 2 = 11 languages. Compatibility runs one way: an Octave-1 voice works on both, but an Octave-2 voice REQUIRES version 2 — so the voice picker flags any voice that doesn't match what you pin here.",
        examples: "(empty) = Hume default  /  1  /  2",
        options: [
          { value: "", label: "Hume default (recommended)" },
          { value: "1", label: "Octave 1 (English / Spanish)" },
          { value: "2", label: "Octave 2 (11 languages)" },
        ],
      },
      {
        key: "COST_HUME_USD_PER_1K_CHARS",
        desc: "USD per 1,000 characters of narration text (Hume Octave's billing unit). Published rates: $0.15 Free/Starter/Creator, $0.12 Pro, $0.10 Scale, $0.05 Business. Defaults to the entry-tier $0.15 — lower it to match your plan.",
        examples: "0.15  /  0.12  /  0.10  /  0.05",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: "RUNWARE_API_KEY",
        desc: "Runware key (EXPERIMENTAL) — one API over many image models (FLUX, Juggernaut, Seedream, Nano Banana). Only used when the AI provider is set to `runware` on the Settings page, where you also pick the model. Never used as a fallback for the other providers.",
        examples: "Sign up at https://runware.ai → API Keys",
      },
      {
        key: "RUNWARE_RETRIES",
        desc: "How many times to retry a Runware generation on a TRANSIENT error (429 queue capacity, 503/504, 5xx, network drop). Permanent errors (bad key, bad model id) fail fast, and a request WE timed out is never retried — it may already have been billed. attempts = retries + 1.",
        examples: "0 = one-shot  ·  3 = default  ·  max 8",
      },
      {
        key: "RUNWARE_CONCURRENCY",
  ...
  },
  {
    ...
    subtitle:
      "Per-unit prices for PAY-AS-YOU-GO providers, used by the Costs page. A rate left at 0 means that provider shows €0.00 however much you use it — the Costs page flags those as “rate not set” rather than free. Defaults are public list prices in USD where one exists; for credit-based vendors (69labs, Magnific, Higgsfield) there is none, so set your own. Changing a rate here restates ALREADY-RECORDED spend, since the Costs page prices from these numbers on every read. ElevenLabs is a subscription: pick your plan on the Costs page under “Billing setup”. HeyGen is pay-as-you-go and IS configured here.",
    fields: [
      {
        key: "COST_USD_TO_EUR",
        label: "USD → EUR rate",
      ...
      },
      {
        ...
        label: "Gemini (standard) — USD per 1M input tokens",
        desc: "Input/prompt price for the STANDARD Gemini tier — any model whose id does not end in -lite. Which rate applies is decided by the model a call actually runs on, not by what the call is for.",
        examples: "3.5 Flash = 1.50",
      },
      {
        key: "COST_GEMINI_OUT_USD_PER_1M",
        label: "Gemini (standard) — USD per 1M output tokens",
        desc: "Output price for the same standard tier.",
        examples: "3.5 Flash = 9.00",
      },
      {
        key: "COST_GEMINI_LITE_IN_USD_PER_1M",
        label: "Gemini (lite) — USD per 1M input tokens",
        desc: "Input/prompt price for the LITE tier (model ids containing -lite, e.g. gemini-3.1-flash-lite). Applies wherever a lite model runs — commonly both the vision scorer and, on many installs, the planner.",
        examples: "3.1 Flash Lite = 0.25",
      },
      {
        key: "COST_GEMINI_LITE_OUT_USD_PER_1M",
        label: "Gemini (lite) — USD per 1M output tokens",
        desc: "Output price for the same lite tier.",
        examples: "3.1 Flash Lite = 1.50",
      },
      {
      ...
      {
        key: "COST_RUNWARE_IMAGE_USD",
        label: "Runware — USD per image (fallback only)",
        desc: "Rarely needed. Runware returns the REAL billed cost of every generation, and that actual amount is always what gets recorded — no estimate involved. This rate is used only if a response arrives without a cost, and defaults to 0 so usage stays auditable at €0 rather than being priced from a guess.",
        examples: "e.g. 0.005 (FLUX.1 dev)",
      },
      {
        key: "COST_LABS69_USD_PER_VIDEO",
        label: "69labs / Grok — USD per generated video",
        desc: "Cost per AI b-roll video clip (counts every regeneration attempt). 69labs bills per account with no public per-clip price, so this defaults to 0 — which means a heavy 69labs install reads as free on the Costs page until you set it.",
        examples: "e.g. 0.15",
      },
      {
        key: "COST_LABS69_IMAGE_USD",
        label: "69labs / Grok — USD per generated image",
        desc: "Cost per AI still from 69labs. Separate from the video rate above: stills used to be priced per-video, so an image was billed at a clip's price.",
        examples: "e.g. 0.02",
      },
      {
        key: "COST_HIGGSFIELD_IMAGE_USD",
        label: "Higgsfield (Soul) — USD per image",
        desc: "Cost per generated Higgsfield still. Credit-based billing with no public per-image price — defaults to 0 until you set your rate.",
        examples: "e.g. 0.02",
      },
      {
        key: "COST_HIGGSFIELD_VIDEO_USD_PER_SEC",
        label: "Higgsfield (DoP) — USD per video-second",
        desc: "Cost per second of generated Higgsfield video. Defaults to 0 until you set your credit-based rate.",
        examples: "e.g. 0.10",
      },
      {
        key: "COST_GROQ_USD_PER_AUDIO_HOUR",
        label: "Groq Whisper — USD per hour of audio",
        desc: "Transcription price, charged whenever word timings are recovered from audio we have no alignment for — every non-ElevenLabs voiceover, and every uploaded one. Billed per hour of AUDIO, not wall-clock time.",
        examples: "whisper-large-v3 = 0.111",
      },
      {
        key: "COST_HEYGEN_USD_PER_MIN",
        label: "HeyGen Avatar IV — USD per minute",
        desc: "Rate for avatar clips rendered with the Avatar IV engine. HeyGen's API is pay-as-you-go (a per-second USD wallet), so this is real metered spend, not a subscription.",
        examples: "e.g. 3.00",
      },
      {
        key: "COST_HEYGEN_UNLIMITED_USD_PER_MIN",
        label: "HeyGen Legacy / \"Unlimited\" — USD per minute",
        desc: "Rate for avatar clips rendered on the legacy v2 engine (no Avatar IV flag). Estimate — confirm against your HeyGen invoice.",
        examples: "e.g. 1.00",
      },
      {
        key: "COST_HEYGEN_TTS_USD_PER_1K_CHARS",
        label: "HeyGen TTS — USD per 1k characters",
        desc: "Narration synthesized by HeyGen — the DEFAULT voice provider. This spend was recorded nowhere until now, so a run narrated by HeyGen reported no voiceover cost at all. No public per-character price, so set yours.",
        examples: "e.g. 0.30",
      },
      {
        key: "COST_LABS69_TTS_USD_PER_1K_CHARS",
        label: "69labs TTS — USD per 1k characters",
        desc: "Narration synthesized by 69labs, billed per character of text.",
        examples: "e.g. 0.30",
      },
      {
        key: "COST_OPENAI_TTS_USD_PER_1K_CHARS",
        label: "OpenAI TTS — USD per 1k characters",
        desc: "Narration synthesized by OpenAI, billed per character of text.",
        examples: "e.g. 0.015",
      },
      {
        key: "COST_MINIMAX_TTS_USD_PER_1K_CHARS",
        label: "MiniMax TTS — USD per 1k characters",
        desc: "Narration synthesized directly by MiniMax, billed per character of text. (MiniMax reached THROUGH AI84 is billed in AI84 credits instead — see the AI84 rate.)",
        examples: "e.g. 0.05",
      },
      {
        key: "COST_GENAIPRO_TTS_USD_PER_1K_CHARS",
        label: "GenAIPro TTS — USD per 1k characters",
        desc: "Narration synthesized by GenAIPro, billed per character of text.",
        examples: "e.g. 0.30",
      },
      {
        key: "COST_STORYBLOCKS_USD_PER_DOWNLOAD",
        label: "Storyblocks — USD per download",
        desc: "Storyblocks is a PAID footage source: resolving a clip's file consumes a download from your plan (the search itself is free). Divide your plan's monthly fee by its included downloads to get an effective rate.",
        examples: "e.g. 0.50",
      },
      {
        key: "COST_GOOGLE_CSE_USD_PER_QUERY",
        label: "Google Custom Search — USD per query",
        desc: "Web image search. Free for the first 100 queries/day and billed above that, so leave this at 0 if you stay inside the free tier — the query COUNT is recorded either way, which is what tells you if you have left it.",
        examples: "$5 per 1000 queries = 0.005",
      },
      {
        key: "COST_HEYGEN_AVATAR_V_USD_PER_MIN",
        label: "HeyGen Avatar V — USD per minute",
        desc: "Rate for avatar clips rendered through the v3 Avatar V engine — the most expensive of the three, so getting it wrong skews a run's cost most. HeyGen does not publish a price for this combination; the default is measured, not official. Settle it from an invoice.",
        examples: "e.g. 4.00",
      },
    ],
  },
