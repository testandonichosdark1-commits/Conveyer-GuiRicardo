...
export default function SettingsPage() {
  ...
  /** Post-OAuth banner. Replaces a blocking alert() the user had to dismiss blind. */
  const [notice, setNotice] = useState<{ kind: "ok" | "error"; autoEnabled: boolean; reason?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    if (!gd) return;
    if (gd === "connected") {
      // autoSync=1 means THIS connection switched auto-upload on (a first-time setup),
      // so the banner can say uploads are now automatic without over-promising on a
      // reconnect, where the toggle is deliberately left as the user had it.
      setNotice({ kind: "ok", autoEnabled: params.get("autoSync") === "1" });
    } else if (gd === "error") {
      setNotice({ kind: "error", autoEnabled: false, reason: params.get("reason") || "unknown error" });
    }
    // Drop the query string but stay on THIS page — it used to rewrite the URL to
    // /settings, a leftover from when this page lived there, which sent a refresh to a
    // page with no Drive section.
    window.history.replaceState({}, "", "/full-settings");
    load(false);
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        ...
        {/* Post-OAuth outcome. Shown here rather than as an alert() so the message sits
            next to the connection status and the toggle it is talking about. */}
        {notice && (
          <div
            style={{
              padding: "12px 14px",
              ...
              marginBottom: 16,
              background: notice.kind === "ok" ? "var(--success-soft)" : "var(--danger-soft)",
            ...
            <div style={{ flex: 1, fontSize: 13, lineHeight: 1.6 }}>
              {notice.kind === "ok" ? (
                <>
                  ...
                  {notice.autoEnabled ? (
                    <div className="muted" style={{ marginTop: 3 }}>
                      Finished videos will now upload automatically. You can disable this at any
                      time with the checkbox below.
                    </div>
                  ) : gdrive?.syncEnabled ? (
                    <div className="muted" style={{ marginTop: 3 }}>
                      Auto-upload is on — finished videos will upload automatically.
                    </div>
                  ) : (
                    <div className="muted" style={{ marginTop: 3 }}>
                      Auto-upload is currently off. Tick{" "}
                      <strong>Auto-upload finished runs to Drive</strong> below (then{" "}
                      <strong>Save all changes</strong>) to upload finished videos automatically.
                    </div>
                  )}
                </>
              ) : (
                <>
                  ...
                  <div className="muted" style={{ marginTop: 3, wordBreak: "break-word" }}>
                    {notice.reason}
                  </div>
                </>
              )}
        ...
        >
          ...
          >
            <input
              ...
              // "0", not "" — unticking is a DELIBERATE choice and has to be
              // distinguishable from "never configured", which is what the first
              // connection is allowed to auto-enable. Writing "" here would make a
              // deliberate opt-out look untouched and let a later reconnect undo it.
              onChange={(e) => setValues({ ...values, GDRIVE_SYNC_ENABLED: e.target.checked ? "1" : "0" })}
