...
const listRuns = db.prepare(
  "SELECT id, title, folder_name, status, created_at, updated_at, output_path, config_json FROM runs WHERE deleted_at IS NULL ORDER BY created_at DESC LIMIT ?"
);
const listRunsBefore = db.prepare(
  "SELECT id, title, folder_name, status, created_at, updated_at, output_path, config_json FROM runs WHERE deleted_at IS NULL AND created_at < ? ORDER BY created_at DESC LIMIT ?"
);
