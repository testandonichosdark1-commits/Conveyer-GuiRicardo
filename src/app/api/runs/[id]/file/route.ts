...
/**
 * A web ReadableStream over a byte range of a file, backed by fs.createReadStream
 * (Node's `end` is INCLUSIVE) so memory stays flat regardless of file size —
 * never buffering the whole file (the old code did `readFileSync`/`Buffer.alloc`,
 * which throws ERR_FS_FILE_TOO_LARGE above ~2 GiB and 500'd on long videos).
 *
 * Disconnect-safe: when the client aborts mid-stream (seeking or closing a
 * <video>, cancelling a download), the consumer cancels this stream. cancel()
 * destroys the fs read so no further 'data' event fires, and every
 * enqueue/close/error is guarded so a late fs callback can never touch the
 * controller after it's closed. That late touch is exactly what threw
 * "Invalid state: Controller is already closed" (ERR_INVALID_STATE) and surfaced
 * as an uncaughtException. Backpressure is preserved with pause/resume so memory
 * stays flat. (Hand-rolled rather than Readable.toWeb, whose internal listeners
 * we couldn't guard against that post-close enqueue.)
 */
function fileStream(target: string, range?: { start: number; end: number }): ReadableStream<Uint8Array> {
  ...
  return new ReadableStream<Uint8Array>({
    start(controller) {
      nodeStream.on("data", (chunk: string | Buffer) => {
        if (done) return;
        try {
          // No encoding is set on the stream, so chunks are always Buffer.
          controller.enqueue(new Uint8Array(chunk as Buffer));
        } catch {
          // Consumer is gone (client disconnected) — stop reading, never throw.
          done = true;
          destroy();
          return;
        }
        // Respect backpressure: pause the fs read until the consumer pulls again.
    ...
    cancel() {
      // Client disconnected / consumer cancelled: tear down the fs read so no
      // later 'data' event enqueues on a closed controller (the ERR_INVALID_STATE
      // cause). Setting done first makes any in-flight callback a no-op.
      done = true;
      destroy();
    },
  });
