...
import { getAvatar, deleteAvatar, ownsHeygenAsset } from "@/lib/avatars";
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  //
  // NEVER for an imported avatar: that talking photo belongs to the operator's
  // HeyGen account and we only reference it — removing it from OUR library must
  // not destroy THEIR asset. Only slots we created are ours to free.
  if (ownsHeygenAsset(avatar)) {
    try {
