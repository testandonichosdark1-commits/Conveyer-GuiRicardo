...
> **Scope note (2026-07-17).** This section documents the **v1/v2** surface, which is
> what Avatar IV and Legacy render on and is still accurate for them. It is no longer
> the whole picture: **Avatar V renders on v3 (`POST /v3/videos`)**, a different request
> model that shares nothing with the `character`/`voice`/`video_inputs` shape below and
> rejects unknown fields outright. The v3 contract is documented in **CLAUDE.md**
> ("Rendering Avatar V") rather than duplicated here, because it was established by
> probing the live API — HeyGen's published docs are wrong about Avatar V — and one
> copy of a hard-won fact is easier to keep true than two.

Auth header on every call: `X-Api-Key: <HEYGEN_API_KEY>` (case-insensitive) — the same
for v1, v2 and v3.
Two API generations coexist; **v1/v2 is fully supported through 2026-10-31** and
matches the `character`/`voice`/`video_inputs` shape — Avatar IV and Legacy build on it.
...
```
(script | uploaded voiceover) + chosen avatar + visual mode (ai|real|mix) + seconds-per-visual + avatar %  ->
 1. narration -> voiceover.mp3 + word timings   (script: ElevenLabs alignment | upload: Groq Whisper)
...
Step 1 is the only step that knows where the narration came from; steps 2–6 consume
the `Voiceover` object and are shared verbatim. See "Uploaded voiceover" below.

---

## Uploaded voiceover (measured 2026-07-22 — supersedes the earlier plan)

An operator may supply a finished narration instead of a script. Design detail and
invariants live in `CLAUDE.md` ("Upload voiceover"); this section records what was
**measured**, because two planning assumptions turned out to be wrong.

### Groq Whisper (`whisper-large-v3`) — verified against the live API

`POST https://api.groq.com/openai/v1/audio/transcriptions`, multipart, `model=whisper-large-v3`,
`response_format=verbose_json`, `timestamp_granularities[]=word` → `words[] {word, start, end}`
(seconds). Input is downmixed to mono/16 kHz/64 kbps mp3 first (~8 kB/s).

- **25 MB request limit** ⇒ ~54 min of that downmix. This, not UX, sets
  `UPLOAD_MAX_MINUTES` (50, leaving margin). Past ~54 min Whisper fails outright.
- **Whisper returns PUNCTUATED tokens** — `"seal,"`, `"nods."`. The plan assumed it
  returned bare words and budgeted a punctuation-restoration stage to keep the
  sentence-aware beat planner working. **That stage was unnecessary and never built.**
  Measured on a real transcript: 8.33% of tokens end a sentence vs 8.83% for the same
  script through ElevenLabs; sd of word duration 238 ms (genuinely varying); token
  accuracy 99.0%.
- Cost ~$0.111 per audio-hour ⇒ ~$0.006 for a 3-minute video.

### The defect this uncovered (fixed by configuring the key)

`alignWords` falls back to `proportionalWords()` when `GROQ_API_KEY` is unset — evenly
spaced, fabricated timings. **All 34 non-ElevenLabs runs in the DB had been using it**
(sd 0.4 ms across word durations, only 2 distinct values — the signature of fabrication),
so every one of those videos had visuals drifting against the narration.

Upload mode therefore **refuses to run without a Groq key** and **throws on zero
transcribed words** rather than reaching that fallback: an uploaded file has no script to
time against, so a fabricated timeline would be the only alternative — and a silently
desynced video is worse than a failed run.
...
## AI provider — kie.ai (default), 69labs, and Magnific AI (all supported)

`AI_PROVIDER` setting (`kie` | `69labs` | `magnific`). kie.ai (`services/kie.ts`):
...
### Magnific AI (`services/magnific.ts`)

One MORE AI b-roll backend — it never replaces kie/69labs. Same async envelope for
every model: `POST /v1/ai/<model>` → `{ data:{ task_id, status } }`; poll
`GET /v1/ai/<model>/{task_id}` → `{ data:{ status, generated:[urls] } }` until
`status:"COMPLETED"` (`CREATED|IN_PROGRESS|COMPLETED|FAILED`). Auth header
`x-magnific-api-key`; base `https://api.magnific.com`. Operations wired:
- **Text→image**: **Mystic** `POST /v1/ai/mystic` `{ prompt, model, resolution, aspect_ratio }`
  (`MAGNIFIC_IMAGE_MODEL` default `realism`, `MAGNIFIC_RESOLUTION` default `2k`) → Ken Burns.
- **Image→image**: same Mystic endpoint + `structure_reference` (base64) — exported
  `editMagnificImageUrl()`; documented + implemented, not currently called by the
  text-driven b-roll pipeline.
- **Text/image→video**: **MiniMax Hailuo 02 1080p** `POST /v1/ai/image-to-video/minimax-hailuo-02-1080p`
  (`MAGNIFIC_VIDEO_MODEL`). Optional `first_frame_image` (base64) = image→video; 1080p
  is a fixed **6-second** clip (assemble trims to the beat length).

Selection & fallback (in `visual-source.ts` `acquireAi`): `AI_PROVIDER=magnific` makes
Magnific the primary (honoring `KIE_AI_MEDIA` image/video/auto exactly like kie); on a
shortfall it falls through to the universal 69labs/Grok floor. When `MAGNIFIC_ENABLED=1`
but Magnific is NOT the chosen provider, it's also tried as a **fallback** after kie/69labs
produce nothing, before that floor — so it's a genuine extra backend in the chain. With
`MAGNIFIC_ENABLED=0` (or no key) the fallback is a no-op and the pipeline is byte-identical
to before. Resilience mirrors Gemini/ElevenLabs: process-wide FIFO concurrency limiter
(`MAGNIFIC_CONCURRENCY`), per-request 120s timeout, and a retry loop (`MAGNIFIC_RETRIES`)
that backs off on transient 429/5xx/timeout/network and fails fast on permanent 4xx.
Cost: `magnific:mystic` (per image) + `magnific:video` (per video-second), both `payg`,
rate operator-configurable via `COST_MAGNIFIC_IMAGE_USD` / `COST_MAGNIFIC_VIDEO_USD_PER_SEC`
(default 0 / estimated — Magnific bills in credits with no public per-unit USD price, so we
record usage and never invent a euro). **Not implemented** (out of scope): TTS/voiceover,
upscaling, relight, style-transfer, music/sound — the API offers them but the pipeline
doesn't need them.
